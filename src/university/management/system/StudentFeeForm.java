...
public class StudentFeeForm extends JFrame implements ActionListener{
  ...
  JLabel l5 = new JLabel("Semester");
	...
	contentPane.add(t3);
       /*String ch = c1.getSelectedItem();
        try{
            conn c = new conn();
            ResultSet rs = c.s.executeQuery("select name,fathers_name from student where rollno = '"+ch+"'");
            while(rs.next()){
  ...
  b3.setForeground(Color.WHITE);
	...
	comboBox_2.setBounds(176, 176, 154, 20);
    ...
    public void actionPerformed(ActionEvent ae){
        try{
            ...
            if(ae.getSource()==b3){
                  ...
                  conn c = new conn();
                  ResultSet r;
                  r = c.s.executeQuery("select * from feeForm where rollno = '"+c1.getSelectedItem()+"';");
                  if(r.next()){
                    setVisible(true);
